...
#[allow(dead_code)]
pub fn orthes(m: &mut Matrix<f64>, h_mat: &mut Matrix<f64>, v_mat: &mut Matrix<f64>) {
    let low = 0;
    let n = m.cols();
    let high = n - 1;
    let mut m = low + 1;
...
#[allow(dead_code)]
fn calc_eigen(m: &mut Matrix<f64>) -> Vec<(f64, f64)>{
    println!("calc_eigen");
    let n = m.cols();
    let mut h_mat = Matrix::new(m.cols(), m.cols(), vec!(0.; m.cols() * m.cols()));
    let mut v_mat = Matrix::new(m.cols(), m.cols(), vec!(0.; m.cols() * m.cols()));
    //let mut ort = vec!(0.;n);
    let mut d = vec!(0.; n);
    let mut e = vec!(0.; n);
    for i in 0..n {
